...
contract("TransitAuthority", accounts => {
  let transitAuthority, costOfSpaceship, commodities
  ...
  beforeEach(async() => {
    ...
    commodities = await CommodityAuthority.new(transitAuthority.address)
    ...
    costOfSpaceship = await transitAuthority.costOfSpaceship()
  })

  it("should allow user to buy a spaceship", async () => {
    const response = await transitAuthority.buySpaceship('Millenium Falcon', { from: player1, value: costOfSpaceship })
    const { tokenId } = response.logs[0].args
    const spaceshipOwner = await transitAuthority.ownerOf(tokenId)
  ...
  it("buying a spaceship without correct amount of ether in transaction will fail", async () => {
    try {
      await transitAuthority.buySpaceship('Millenium Falcon', { from: player1, value: costOfSpaceship.sub(1) })
    } catch (e) {
      return assert(true)
    }
  ...
  it("should only let users buy one spaceship (for now)", async () => {
    await transitAuthority.buySpaceship('Millenium Falcon', { from: player1, value: costOfSpaceship })
    try {
      await transitAuthority.buySpaceship('Millenium Falcon2', { from: player1 })
    } catch (e) {
      return assert(true)
    }
  ...
  it("should allow a player to travel to the planets with ids 0 to 6, inclusive", async () => {
    await transitAuthority.buySpaceship('Millenium Falcon', { from: player1, value: costOfSpaceship })
    for (let i = 0; i < 7; i++) {
      await transitAuthority.travelToPlanet(i, { from: player1 })
      const response = await transitAuthority.getInfo({ from: player1 })
      assert.equal(response[1].toString(), i.toString(), "didn't change planet")

      const refuelCost = await transitAuthority.refuelCost()
      await transitAuthority.refuel({ from: player1, value: refuelCost })
    }
  })
  ...
  it("travelling to a planet uses fuel", async () => {
    await transitAuthority.buySpaceship('Millenium Falcon', { from: player1, value: costOfSpaceship })
    let checkFuel = await transitAuthority.checkFuel(player1)
    const fuelBefore = checkFuel[0]
    await transitAuthority.travelToPlanet(1, { from: player1 })
    checkFuel = await transitAuthority.checkFuel(player1)
  ...
  it("cannot refuel spaceship if no ether supplied", async () => {
    ...
    await transitAuthority.travelToPlanet(1, { from: player1 })
    try {
      await transitAuthority.refuel({ from: player1 })
    } catch (e) {
      return assert(true)
    }
  ...
  it("cannot refuel spaceship if not enough ether supplied", async () => {
    ...
    const refuelCost = await transitAuthority.refuelCost()
    try {
      await transitAuthority.refuel({ from: player1, value: refuelCost.sub(1) })
    } catch (e) {
      return assert(true)
    }
    assert(false, "no error when trying to refuel")
  ...
  it("should error if player tries to travel to a planet that doesn't exist", async () => {
    ...
    await transitAuthority.buySpaceship('Millenium Falcon', { from: player1, value: costOfSpaceship })
    try {
      await transitAuthority.travelToPlanet(7, { from: player1 })
    } catch (e) {
      errorCount++
    }
    try {
      await transitAuthority.travelToPlanet(128, { from: player1 })
    } catch (e) {
      errorCount++
    }
  ...
  it("allows user to get planet's URI", async () => {
    const num1 = '1042'
    const uri1 = await transitAuthority.planetURI(num1, { from: player1 })
    ...
    const uri2 = await transitAuthority.planetURI(num2, { from: player1 })
